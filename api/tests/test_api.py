...
async def test_lifespan_model_loaded_successfully(
    ...
    async with lifespan(mock_app): # Call the lifespan context manager directly
        from api.app.main import model, expected_model_columns # Import after lifespan context
...
def test_predict_attrition_no_id_employee(client: TestClient):
    ...
    employee_data_dict = {
        "age": 30, "genre": 1, "revenu_mensuel": 5000.0, "statut_marital": "Célibataire",
        "departement": "R&D", "poste": "Développeur", "nombre_experiences_precedentes": 2,
        "annee_experience_totale": 8, "annees_dans_l_entreprise": 4, "annees_dans_le_poste_actuel": 2,
        "nombre_participation_pee": 1, "nb_formations_suivies": 2, "nombre_employee_sous_responsabilite": 0,
        "distance_domicile_travail": 10, "niveau_education": 3, "domaine_etude": "Informatique",
    ...
    predictions = response.json()["predictions"]
    assert len(predictions) == 1
    assert predictions[0]["id_employee"] == 0 # Expecting temporary ID 0
...
def test_predict_attrition_missing_feature_in_input(client: TestClient):
    ...
    with patch('api.app.main.expected_model_columns', new=['id_employee', 'age', 'dummy_feature']):
