...
LOG_ODDS_RISK_THRESHOLDS = {
    "Low": (-np.inf, _logit(0.3)),
    "Medium": (_logit(0.3), _logit(0.7)),
    "High": (_logit(0.7), np.inf),
}
...
def _clean_extrait_eval(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in [
        ...
        if col in df.columns:
            if col == "heure_supplementaires":
                df.rename(columns={"heure_supplementaires": "heures_supplementaires"}, inplace=True)
...
def _clean_extrait_sirh(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in ["nombre_heures_travailless"]: # Placeholder for columns to drop
        if col in df.columns:
...
def load_and_merge_data(
    ...
    # The original app.py had '...' as a column to drop, which is likely a placeholder or error.
    # I'm removing it here to avoid potential issues.
...
def clean_and_engineer_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    _, _, cat_cols_from_model = preprocessor.transformers_[1]
...
def _get_risk_category_from_log_odds(log_odds: float) -> str:
    ...
    low_threshold = LOG_ODDS_RISK_THRESHOLDS["Low"][1]
    high_threshold = LOG_ODDS_RISK_THRESHOLDS["High"][0]
...
def generate_shap_report_data(
    ...
    all_shap_values = explainer.shap_values(x_transformed_for_shap)
