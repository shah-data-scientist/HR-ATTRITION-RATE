...
synthetic_data = {
    ...
    "augementation_salaire_precedente": np.round(
        np.random.uniform(0.0, 35.0, NUM_EMPLOYEES), 1
    ),
    ...
    "statut_marital": np.random.choice(
        ["Célibataire", "Marié", "Divorcé"], NUM_EMPLOYEES
    ),
    "departement": np.random.choice(["IT", "HR", "Sales", "Marketing"], NUM_EMPLOYEES),
    "poste": np.random.choice(
        ["Développeur", "Manager", "Analyste", "Support"], NUM_EMPLOYEES
    ),
    "domaine_etude": np.random.choice(
        ["Informatique", "RH", "Commerce", "Autre"], NUM_EMPLOYEES
    ),
    "frequence_deplacement": np.random.choice(
        ["Rarement", "Souvent", "Jamais"], NUM_EMPLOYEES
    ),
    ...
    "code_sondage": [
        str(i + 1) for i in range(NUM_EMPLOYEES)
    ],  # Assuming string for code_sondage
...
print(
    f"Generated {NUM_EMPLOYEES} synthetic employee records in 'data/synthetic_employees.csv'."
)
