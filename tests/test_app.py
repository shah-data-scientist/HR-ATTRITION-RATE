...
from app import _clean_extrait_eval, _clean_extrait_sirh, _clean_extrait_sondage, load_and_merge_data, clean_and_engineer_features, get_risk_category, RISK_THRESHOLDS, load_model_and_data, display_confusion_matrix_and_metrics, generate_shap_html_report
...
def sample_sirh_df():
    return pd.DataFrame({
        'id_employee': [1, 2, 3, 5],
        'genre': ['M', 'F', 'm', 'f'],
        'nombre_heures_travailless': [160, 150, 170, 180] # This column should be dropped
    })
...
def mock_model_pipeline():
    ...
    mock_logreg_model.predict_proba.return_value = np.array([[0.2, 0.8]]) # Example probability
...
def test_load_and_merge_data_merge_logic(sample_eval_df, sample_sirh_df, sample_sondage_df):
    merged_df = load_and_merge_data(sample_eval_df.copy(), sample_sirh_df.copy(), sample_sondage_df.copy())
    ...
    assert merged_df.loc[merged_df['id_employee'] == 4, 'satisfaction_employee_equipe'].iloc[0] == 3
    assert pd.isna(merged_df.loc[merged_df['id_employee'] == 3, 'satisfaction_employee_equipe'].iloc[0])
...
def test_clean_and_engineer_features_improvement_evaluation(merged_df_for_feature_engineering):
...
def test_clean_and_engineer_features_total_satisfaction(merged_df_for_feature_engineering):
    df = clean_and_engineer_features(merged_df_for_feature_engineering.copy())
...
def test_get_risk_category_medium():
    assert get_risk_category(0.45, 0.5) == "Medium" # 0.45 is threshold - buffer, and >= min_medium_prob
    assert get_risk_category(0.5, 0.5) == "Medium" # 0.5 is between threshold - buffer and threshold + buffer, and >= min_medium_prob
    assert get_risk_category(0.54, 0.5) == "Medium" # 0.54 is between threshold - buffer and threshold + buffer, and >= min_medium_prob
...
def test_get_risk_category_edge_cases():
    ...
    assert get_risk_category(0.49, 0.5) == "Medium" # Between threshold - buffer and threshold + buffer, and >= min_medium_prob
    assert get_risk_category(0.51, 0.5) == "Medium" # Between threshold - buffer and threshold + buffer, and >= min_medium_prob
    assert get_risk_category(0.55, 0.5) == "High" # At threshold + buffer
    assert get_risk_category(0.19, 0.5) == "Low" # Below min_medium_prob, so defaults to Low
...
def test_display_confusion_matrix_and_metrics(
    mock_st_pyplot, mock_tight_layout, mock_xticks, mock_yticks, mock_gca,
    mock_sns_heatmap, mock_subplots, mock_st_write, mock_st_columns, mock_st_subheader):
