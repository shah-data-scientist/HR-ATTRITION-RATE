...
cur.execute("""
    ...
    ORDER BY trace_id
""")
print(f"{'Trace ID':<10} {'Input ID':<10} {'Output ID':<11} {'Version':<8} {'Source':<8} {'Created At':<25}")
print("-" * 80)
for row in cur.fetchall():
    print(f"{row[0]:<10} {row[1]:<10} {row[2]:<11} {row[3]:<8} {row[4]:<8} {str(row[5]):<25}")
...
""")
print(f"{'Employee ID':<12} {'Age':<5} {'Genre':<7} {'Salary':<8} {'Dept':<12} {'Poste':<20} {'User ID':<8}")
print("-" * 80)
for row in cur.fetchall():
    print(f"{row[0]:<12} {row[1]:<5} {row[2]:<7} {row[3]:<8} {row[4]:<12} {row[5]:<20} {row[6]:<8}")
...
cur.execute("""
    ...
    ORDER BY t.trace_id
""")
print(f"{'Trace':<7} {'Employee':<10} {'Probability':<12} {'Label':<10} {'Risk':<10} {'SHAP?':<8}")
print("-" * 80)
for row in cur.fetchall():
    prob_pct = f"{row[2]*100:.2f}%"
    print(f"{row[0]:<7} {row[1]:<10} {prob_pct:<12} {row[3]:<10} {row[4]:<10} {row[5]:<8}")
