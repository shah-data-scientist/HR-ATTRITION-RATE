...
class TestDataUploadAndPrediction:
    ...
    def test_prediction_structure(self, api_client, test_data):
        ...
        assert prediction["prediction"] in ["Stay", "Leave"], (
        ...
        print(f"✓ Prediction structure validated for employee {prediction['id_employee']}")
    ...
    def test_shap_values_present(self, api_client, test_data):
        ...
        assert isinstance(prediction["base_value"], (int, float)), "base_value should be numeric"
        assert isinstance(prediction["feature_names"], list), "feature_names should be list"
        
        # Validate consistency
        assert len(prediction["shap_values"]) == len(prediction["feature_names"]), (
...
class TestFullDataset:
    ...
    def test_predict_with_full_dataset(self, api_client, test_data):
        ...
        print(f"  Predicted to Leave: {leave_count} ({leave_count/len(predictions)*100:.1f}%)")
        print(f"  Predicted to Stay: {stay_count} ({stay_count/len(predictions)*100:.1f}%)")
        print(f"\nRisk Distribution:")
