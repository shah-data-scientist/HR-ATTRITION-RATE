...
class TestDataProcessingEdgeCases:
    ...
    def test_clean_genre_edge_cases(self):
        ...
        df = pd.DataFrame({
            'genre': ['M', 'm', 'F', 'f', 'Homme', 'homme', 'Femme', 'femme',
                     'Male', 'male', 'Female', 'female', 1, 0]
        })
    ...
    def test_clean_heures_with_various_formats(self):
        ...
        df = pd.DataFrame({
            'heure_supplementaires': ['Oui', 'oui', 'Non', 'non', 'Yes', 'yes',
                                     'No', 'no', 'True', 'true', 'False', 'false']
        })
        result = clean_raw_input(df)
    ...
    def test_salary_increase_various_formats(self):
        ...
        assert all('%' not in str(val) for val in result['augementation_salaire_precedente'])
...
class TestPreprocessingEdgeCases:
    ...
    def test_enforce_schema_with_string_categoricals(self):
        ...
        df = pd.DataFrame({col: ['test_value'] if col in CATEGORICAL_COLS else [0] 
...
class TestDataMerging:
    ...
    def test_merge_with_duplicate_columns(self):
        ...
        df2 = pd.DataFrame({'id_employee': [1, 2], 'age': [25, 30], 'value': [100, 200]})
...
class TestDataTypeConversions:
    ...
    def test_categorical_to_string(self):
        ...
        df = pd.DataFrame({
            'departement': ['IT', 'HR', 'Sales'],
            'poste': ['Dev', 'Manager', 'Rep']
        })
