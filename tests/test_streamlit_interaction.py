...
class TestStreamlitInteraction(unittest.TestCase):
    ...
    def test_app_flow_with_synthetic_data(self):
        ...
        at.set_uploaded_files(
            file_uploader_widget,
            [self.eval_file, self.sirh_file, self.sondage_file],
        ...
        self.assertFalse(
            predict_button_run_result.exception,
            f"App raised an exception: {predict_button_run_result.exception}",
        )
        ...
        self.assertEqual(
            len(error_messages),
            0,
            f"Found error messages in app output: {error_messages}",
        )

        # Optionally, check for success messages or specific content
        self.assertIn(
            "Predictions received successfully!",
            predict_button_run_result.success[0].body,
        )
