...
recent_inputs = db.query(ModelInput).filter(ModelInput.prediction_timestamp >= cutoff).count()
recent_outputs = db.query(ModelOutput).filter(ModelOutput.prediction_timestamp >= cutoff).count()
recent_traces = db.query(PredictionTraceability).filter(PredictionTraceability.created_at >= cutoff).count()
...
if recent_inputs > 0:
    latest = db.query(ModelInput).order_by(ModelInput.prediction_timestamp.desc()).first()
