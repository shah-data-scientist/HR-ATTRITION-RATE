...
def _plot_probability_waterfall_topk(
    ...
    ax.axvline(
        ...
        linestyle="--",
        linewidth=1.5,
        label=f"Average: {avg_prob*100:.0f}%",
    )
    ax.axvline(
        employee_prob * 100.0,
        color="black",
        linestyle="-.",
        linewidth=1.8,
        label=f"This employee: {employee_prob*100:.0f}%",
    )
    ...
    ax.legend(
        by_label.values(), by_label.keys(), loc="lower right", fontsize=10, frameon=True
    )
...
def _setup_app():
    ...
    model, X_train_loaded, y_train_loaded, X_test_loaded, y_test_loaded = (
        load_model_and_data()
    )
...
def main():
    ...
    with col_t1:
        display_confusion_matrix_and_metrics(
            y_train_loaded, train_prediction_proba, 0.3, "Threshold 0.30"
        )
    with col_t2:
        display_confusion_matrix_and_metrics(
            y_train_loaded, train_prediction_proba, 0.5, "Threshold 0.50 (Default)"
        )
    with col_t3:
        display_confusion_matrix_and_metrics(
    ...
    if raw_uploaded_data is not None:
        if st.button("Predict Attrition Risk"):
            ...
            report_data["Risk_Attrition"] = report_data[
                "Attrition_Risk_Percentage"
            ].apply(lambda x: get_risk_category(x, main_threshold))
